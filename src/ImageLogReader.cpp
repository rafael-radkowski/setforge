...
bool ImageLogReader::Read(string path_and_file, vector<ImageLog>* log )
{
	...
	while (in)
	{
		...
		if (items.size() > 0) {
			...
			glm::vec3 pos(atof(items[5].c_str()), atof(items[6].c_str()), atof(items[7].c_str()));
			glm::quat q(atof(items[8].c_str()), atof(items[9].c_str()), atof(items[10].c_str()), atof(items[11].c_str()));

			log->push_back(ImageLog(atoi(items[0].c_str()), items[1], items[2], items[3], items[4], pos, q));
		}
	}
